    ...
    try {
      ...
      if (!response.ok) {
        ...
        setResult({
          success: false,
          domain: cleanDomain,
          error: data.error || data.message || 'Failed to check SSL certificate'
        })
        return
      }
      ...
      if (data.certificate?.valid) {
        ...
        toast.warning('SSL certificate has expired!')
      } else {
        toast.info('SSL check completed')
      }
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground">
            Verify SSL/TLS certificate status, validity, and security details for any domain.
          </p>
        </div>
                    ...
                    <CardContent className="py-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm text-muted-foreground">Certificate Expires In</p>
                        ...
                        {result.certificate.daysRemaining <= 30 && result.certificate.daysRemaining > 0 && (
                          <div className="flex items-center gap-2 text-orange-600">
                            <Warning size={24} />
                            <span className="text-sm font-medium">Renew Soon!</span>
                          </div>
                        )}
                      </div>
                    </CardContent>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {result.certificate.cipher && (
                          <div className="space-y-1">
                            <p className="text-sm text-muted-foreground">Cipher</p>
                      ...
                      {result.certificate.altNames && result.certificate.altNames.length > 0 && (
                        <div className="pt-4 border-t">
                          <p className="text-sm text-muted-foreground mb-2 flex items-center gap-2">
                            <Globe size={16} />
                            Subject Alternative Names ({result.certificate.altNames.length})
                          </p>
                          <div className="flex flex-wrap gap-2">
                            ...
                            {result.certificate.altNames.length > 10 && (
                              <span className="px-2 py-1 bg-muted rounded text-sm text-muted-foreground">
                                +{result.certificate.altNames.length - 10} more
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      {result.certificate.fingerprint256 && (
                        <div className="pt-4 border-t">
                          <p className="text-sm text-muted-foreground mb-2">SHA-256 Fingerprint</p>
